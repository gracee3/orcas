...
TT is a local-first orchestration system for supervising agent workflows on a single machine. The daemon is the authority for workflow state, lifecycle transitions, and local IPC. The CLI is a client of that daemon.

The control plane stays local. TT owns the records that matter for supervision, while TT remains the execution substrate underneath those records. That separation keeps workflow state explicit and inspectable, and it gives the daemon a single place to coordinate startup, persistence, and event delivery.

## Runtime Roles

`tt` is the operator-facing CLI. It is used for daemon lifecycle commands, status inspection, workflow review, and other supervisor actions that need to go through the daemon. For planning hierarchy CRUD, the CLI converges on the same authority surface used for canonical planning work.

`ttd` is the long-lived service process. On startup it resolves configuration, ensures the runtime and data directories exist, writes runtime metadata, binds the local socket, and connects to the upstream TT app-server. From that point on it serves local clients and owns the live in-memory view of TT state.

## State and Communication

TT uses a local Unix domain socket for IPC. The wire format is structured JSON-RPC 2.0, exchanged as line-delimited JSON messages. Clients use requests for commands and queries, responses for returned data, and notifications for state-change events.
...
The daemon’s state model is TT-native, and durable TT state now lives in one local SQLite store. Authority-owned workstreams, work units, and tracked threads are stored in `state.db` with explicit commands, revisions, and tombstones. Collaboration/runtime snapshots, live thread mirrors, live turn mirrors, and workstream runtime summaries are also persisted in `state.db`. `state/get` remains a merged derived snapshot rather than a raw table dump, while `authority/hierarchy/get` and authority detail RPCs remain the canonical planning hierarchy surfaces.
...
Thread and turn state describe the TT-side execution view that TT supervises. A thread may be started or resumed, attached or detached, and observed through turn history and live events. Turns may be steered, interrupted, or allowed to complete. The daemon keeps enough state to answer what is active, what is terminal, and what is only queryable as historical data.

Reports and decisions close the loop. Worker reports and supervisor decisions are recorded back into TT state, and the daemon emits lifecycle events so the CLI can react without reconstructing history from raw upstream traffic.

## Execution Model

The daemon owns the upstream TT connection and the local supervision state. Supervisors do not own either one. They connect to `ttd` on demand, ask for state, and issue commands through the daemon’s API surface.
...
This separation matters operationally. Restarting the CLI does not affect the TT connection. Restarting the daemon does, because it owns the live upstream session and the supervised state surfaces.

## Design Principles

TT follows a small set of consistent rules:

1. Local-first: state, runtime metadata, and IPC stay on the host.
2. Separation of control and execution: the supervisor controls, the daemon orchestrates, and TT executes.
