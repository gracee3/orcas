...
- `orcas tui` is the checked-in tabbed dashboard wrapper for launching and resuming live upstream Codex TUI sessions against selected threads

The dashboard wrapper can be closed independently of the Codex child sessions it launches. If you resume a thread into the Codex TUI, that child process stays separate and keeps running until you terminate it directly. The dashboard starts on a blank canvas with a border HUD, fades the HUD in and out on `F2`, and keeps tab switching available for live sessions.
