...
`orcas tui` opens directly into a supervisor-backed Codex session. It reuses a remembered supervisor thread when one exists for the current Orcas home, or creates one rooted at `~/.orcas` when needed, then attaches the dashboard wrapper to that session. It launches the upstream Codex TUI against the shared app-server with `codex --remote <ws-url> resume <THREAD>`, switchable tabs, and a border HUD with an explicit shortcut legend on its own visible line. The UI no longer renders left/right workstream or thread sidebars, and the startup canvas is the supervisor session rather than a blank screen; when the HUD is hidden, the canvas shows a `press F2 for HUD` hint.
