...
use orcas_core::{OrcasError, OrcasResult, ReconnectPolicy};
...
impl CodexTransport for WebSocketTransport {
    async fn connect(&self) -> OrcasResult<TransportConnection> {
        let (socket, response) = connect_async(&self.endpoint)
            .await
            .map_err(|error| OrcasError::Transport(format!("websocket connect failed: {error}")))?;
...
#[cfg(test)]
mod tests {
    use orcas_core::ReconnectPolicy;
