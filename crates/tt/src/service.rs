...
use tt_core::{
    ...
    TT_APP_SERVER_LISTEN_URL_ENV, TT_APP_SERVER_OWNER_KIND_ENV, TT_APP_SERVER_OWNER_PID_ENV,
...
use crate::streaming::{
    ConsoleReporter, RetryPolicy, StreamReporter, StreamingCommandRunner,
    TTSupervisorStreamingBackend,
};
...
pub use ttd::TTRuntimeOverrides as RuntimeOverrides;

const SUPERVISOR_CLI_OPERATOR: &str = "supervisor_cli_operator";
const TT_CLI_NODE_ID: &str = "tt-cli";
...
fn print_landing_authorization(authorization: &tt_core::LandingAuthorizationRecord) {
...
fn print_landing_execution(execution: &tt_core::LandingExecutionRecord) {
...
fn print_prune_workspace_operation(operation: &tt_core::WorkspaceOperationRecord) {
...
fn print_planning_session(session: &tt_core::PlanningSession) {
...
fn print_planning_revision_proposal(proposal: &tt_core::planning::PlanRevisionProposal) {
...
#[async_trait]
trait SupervisorTTBackend {
    async fn tt_assignment_list(
        &self,
        params: &ipc::TTAssignmentListRequest,
    ) -> Result<ipc::TTAssignmentListResponse>;
...
#[async_trait]
impl SupervisorTTBackend for TTIpcClient {
    async fn tt_assignment_list(
        &self,
        params: &ipc::TTAssignmentListRequest,
    ) -> Result<ipc::TTAssignmentListResponse> {
        TTIpcClient::tt_assignment_list(self, params)
            .await
            .map_err(Into::into)
    }
    ...
    async fn supervisor_decision_list(
        ...
        TTIpcClient::supervisor_decision_list(self, params)
            .await
            .map_err(Into::into)
    }
    ...
    async fn supervisor_decision_get(
        ...
        TTIpcClient::supervisor_decision_get(self, params)
            .await
            .map_err(Into::into)
    }
    ...
    async fn supervisor_decision_propose_steer(
        ...
        TTIpcClient::supervisor_decision_propose_steer(self, params)
            .await
            .map_err(Into::into)
    }
    ...
    async fn supervisor_decision_replace_pending_steer(
        ...
        TTIpcClient::supervisor_decision_replace_pending_steer(self, params)
            .await
            .map_err(Into::into)
    }
    ...
    async fn supervisor_decision_record_no_action(
        ...
        TTIpcClient::supervisor_decision_record_no_action(self, params)
            .await
            .map_err(Into::into)
    }
    ...
    async fn supervisor_decision_manual_refresh(
        ...
        TTIpcClient::supervisor_decision_manual_refresh(self, params)
            .await
            .map_err(Into::into)
    }
    ...
    async fn supervisor_decision_approve_and_send(
        ...
        TTIpcClient::supervisor_decision_approve_and_send(self, params)
            .await
            .map_err(Into::into)
    }
    ...
    async fn supervisor_decision_reject(
        ...
        TTIpcClient::supervisor_decision_reject(self, params)
            .await
            .map_err(Into::into)
    }
}
...
pub struct SupervisorService {
    ...
    daemon: TTDaemonProcessManager,
    overrides: TTRuntimeOverrides,
}

#[derive(Debug, Clone)]
struct LocalTTAppServer {
    endpoint: String,
...
#[derive(Debug, Clone)]
enum ResolvedTTWorktreeCleanupTarget {
    Workstream {
        workstream: authority::WorkstreamSummary,
        repo_root: PathBuf,
        worktree_path: PathBuf,
...
impl SupervisorService {
    pub async fn load(overrides: &RuntimeOverrides) -> Result<Self> {
        ...
        let daemon = TTDaemonProcessManager::new(paths.clone(), overrides.clone());
    ...
    fn tt_role_pack_source_root() -> PathBuf {
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../.tt")
    }

    fn tt_role_pack_target_root(home_root: &Path) -> PathBuf {
        home_root.join("tt-home/.tt")
    }

    fn shared_app_server_auth_source(&self) -> Result<Option<PathBuf>> {
        if let Some(auth_file) = self.config.tt.direct_api.auth_file.as_ref() {
            if auth_file.is_file() {
                return Ok(Some(auth_file.clone()));
            }
        }
        let fallback = Self::home_dir()?.join(".tt/auth.json");
        if fallback.is_file() {
            Ok(Some(fallback))
        } else {
            Ok(None)
    ...
    pub fn shared_app_server_auth_file(&self) -> PathBuf {
        self.shared_app_server_tt_home().join("auth.json")
    }
    ...
    fn shared_app_server_config_file(&self) -> PathBuf {
        self.shared_app_server_tt_home().join("config.toml")
    }
    ...
    fn copy_directory_tree(source: &Path, target: &Path) -> Result<()> {
        if !source.is_dir() {
            bail!("tt role pack source {} does not exist", source.display());
        }
    ...
    async fn sync_tt_role_pack(&self, name: &str) -> Result<PathBuf> {
        self.ensure_default_app_server_name(name)?;
        let home_root = self.app_server_home_root(name);
        let source_root = Self::tt_role_pack_source_root();
        let target_root = Self::tt_role_pack_target_root(&home_root);
    ...
    async fn print_app_server_configuration(&self, name: &str, config: &AppConfig) -> Result<()> {
        let configured_endpoint = config.tt.effective_listen_url().to_string();
        let servers = Self::discover_local_tt_app_servers().await?;
        let matching = servers
            .iter()
            .filter(|server| server.endpoint == configured_endpoint)
            .collect::<Vec<_>>();
        let app_server = config.tt.effective_app_server();
        let home_root = self.app_server_home_root(name);
        let role_pack_root = Self::tt_role_pack_target_root(&home_root);
        ...
        println!("binary_path: {}", config.tt.binary_path.display());
        println!("tt_home: {}", home_root.join("tt-home").display());
        println!(
            "tt_auth: {}",
        ...
        println!("tt_role_pack_root: {}", role_pack_root.display());
        println!("sqlite_home: {}", home_root.join("sqlite-home").display());
        println!(
            "responses_base_url: {}",
            config.tt.responses.base_url.as_deref().unwrap_or("unset")
        );
        println!(
    ...
    fn local_roles_root() -> Result<PathBuf> {
        Ok(Self::home_dir()?.join(".tt/roles"))
    }

    fn default_worktree_root() -> Result<PathBuf> {
        Ok(Self::home_dir()?.join("worktrees/tt"))
    }
    ...
    pub async fn lane_init(&self, label: &str, repo_specs: &[String]) -> Result<()> {
        ...
        write_toml(
            &lane_paths.manifest_file,
            &LaneManifest::new(
                label,
                &slug,
                lane_paths.root.display().to_string(),
                lane_paths.shared_home_dir.display().to_string(),
                lane_paths.repos_dir.display().to_string(),
                lane_paths.worktrees_dir.display().to_string(),
                lane_paths.runtime_dir.display().to_string(),
            ),
        )?;
    ...
    async fn workstream_add_internal(
        ...
        let lane_root = worktree_path.join(".tt");
        let tt_home = lane_root.join("tt-home");
        let sqlite_home = lane_root.join("sqlite-home");
        tokio::fs::create_dir_all(&tt_home).await?;
        ...
        let execution_scope = build_execution_scope(
            Some(tt_home.display().to_string()),
    ...
    async fn resolve_workstream_selector(&self, selector: &str) -> Result<ResolvedSpawnWorkstream> {
        ...
        let worktree_path = Self::derive_worktree_path_from_tt_home(Path::new(&scope.tt_home))
            .ok_or_else(|| {
                anyhow!(
                    "unable to derive worktree path from tt_home {}",
                    scope.tt_home
                )
            })?;
    ...
    fn derive_worktree_path_from_tt_home(tt_home: &Path) -> Option<PathBuf> {
        let parent = tt_home.parent()?;
        if parent
            .file_name()
            .is_some_and(|value| value.to_string_lossy() == ".tt")
        {
    ...
    async fn resolve_cleanup_target_from_workstream(
        &self,
        workstream: authority::WorkstreamSummary,
    ) -> Result<ResolvedTTWorktreeCleanupTarget> {
        let scope = workstream.execution_scope.as_ref().ok_or_else(|| {
            anyhow!(
                "workstream `{}` does not have an execution scope yet",
                workstream.title
        ...
        Ok(ResolvedTTWorktreeCleanupTarget::Workstream {
            workstream,
            repo_root,
            worktree_path,
            branch_name,
    ...
    async fn resolve_cleanup_target_from_tracked_thread(
        ...
        Ok(ResolvedTTWorktreeCleanupTarget::TrackedThread {
    ...
    async fn resolve_tt_worktree_cleanup_target(
        &self,
        selector: &str,
    ) -> Result<ResolvedTTWorktreeCleanupTarget> {
    ...
    pub async fn doctor(&self) -> Result<()> {
        ...
        println!("tt_bin: {}", self.config.tt.binary_path.display());
        println!("tt_endpoint: {}", self.config.tt.effective_listen_url());
        println!("connection_mode: {:?}", self.config.tt.connection_mode);
        println!(
            "app_server_owner: {:?}",
            self.config.tt.effective_app_server().owner
        );
        ...
        if lanes_root.exists() {
            for lane_entry in fs::read_dir(&lanes_root)? {
                ...
                let lane_manifest_file = lane_root.join("lane.toml");
                if let Some(manifest) = read_toml::<LaneManifest>(&lane_manifest_file)? {
    ...
    pub async fn daemon_status(&self) -> Result<()> {
        ...
        if let Some(status) = socket_status.daemon_status.as_ref() {
            println!("tt_endpoint: {}", status.tt_endpoint);
            println!("tt_binary: {}", status.tt_binary_path);
    ...
    pub async fn app_server_add(&self, name: &str) -> Result<()> {
        ...
        config.tt.app_server.default.enabled = true;
        self.persist_config(&config).await?;
        let home_root = self.app_server_home_root(name);
        tokio::fs::create_dir_all(home_root.join("tt-home")).await?;
        tokio::fs::create_dir_all(home_root.join("sqlite-home")).await?;
        let role_pack_root = self.sync_tt_role_pack(name).await?;
        let auth_file = self.prepare_shared_app_server_auth()?;
        info!(
            surface = "cli",
            action = "app_server_add",
            app_server = name,
            listen_url = %config.tt.effective_listen_url(),
            owner = ?config.tt.effective_app_server().owner,
    ...
    pub async fn app_server_remove(&self, name: &str) -> Result<()> {
        self.ensure_default_app_server_name(name)?;
        let mut config = self.config.clone();
        if config.tt.effective_app_server().owner == TTAppServerOwner::TT {
            let _ = self.app_server_stop(name).await;
        }
        config.tt.app_server.default.enabled = false;
        self.persist_config(&config).await?;
        info!(
            surface = "cli",
        ...
        println!("listen_url: {}", config.tt.effective_listen_url());
        Ok(())
    }
    ...
    pub async fn app_server_info(&self, name: &str) -> Result<()> {
        ...
        self.print_app_server_configuration(name, &self.config)
            .await?;
        let configured_endpoint = self.config.tt.effective_listen_url().to_string();
        let servers = Self::discover_local_tt_app_servers().await?;
    ...
    pub async fn app_server_start(&self, name: &str) -> Result<()> {
        ...
        let app_server = config.tt.effective_app_server().clone();
        if !app_server.enabled {
            bail!("app-server `{name}` is disabled; run `tt app-server add {name}` first");
        }
        if app_server.owner != TTAppServerOwner::TT {
            bail!(
                "app-server `{name}` is owned by {:?}; use that owner to start it",
                app_server.owner
            );
        }
        if app_server.transport != TTAppServerTransport::Websocket {
            bail!(
                "app-server `{name}` uses {:?}; TT-managed start currently supports websocket listeners only",
        ...
        let role_pack_root = self.sync_tt_role_pack(name).await?;
        let auth_file = self.prepare_shared_app_server_auth()?;
        let listen_url = config.tt.effective_listen_url().to_string();
        let existing = Self::discover_local_tt_app_servers()
            .await?
            .into_iter()
        ...
        if let Some(server) = existing {
            ...
            println!("tt_role_pack: installed");
            println!("tt_role_pack_root: {}", role_pack_root.display());
            if let Some(auth_file) = auth_file.as_ref() {
                println!("tt_auth_file: {}", auth_file.display());
            } else {
                println!("tt_auth_file: unset");
            }
            return Ok(());
        }

        let tt_home = home_root.join("tt-home");
        ...
        let mut command = Command::new(&config.tt.binary_path);
        command.kill_on_drop(false);
        command.arg("app-server");
        command
            .arg("--listen")
            .arg(config.tt.effective_listen_url());
        for override_arg in &config.tt.config_overrides {
            command.arg("--config").arg(override_arg);
        }
        command.env("RUNTIME_HOME", &tt_home);
        command.env("RUNTIME_SQLITE_HOME", &sqlite_home);
        ...
        let mut child = command.spawn().map_err(|error| {
            anyhow!(
                ...
                config.tt.binary_path.display(),
                config.tt.effective_listen_url()
            )
        })?;
        ...
        for _ in 0..40 {
            if Self::discover_local_tt_app_servers()
                .await?
                .into_iter()
    ...
    pub async fn app_server_stop(&self, name: &str) -> Result<()> {
        ...
        let listen_url = config.tt.effective_listen_url().to_string();
        let Some(server) = Self::discover_local_tt_app_servers()
            .await?
            .into_iter()
            .find(|server| server.endpoint == listen_url)
        else {
    ...
    pub fn shared_app_server_tt_home(&self) -> PathBuf {
        self.app_server_home_root("default").join("tt-home")
    }
    ...
    pub async fn daemon_discover_app_servers(&self) -> Result<()> {
        let servers = Self::discover_local_tt_app_servers().await?;
        if servers.is_empty() {
            println!("no local tt app-server listeners discovered");
            return Ok(());
        }
    ...
    pub async fn daemon_start(&self, force: bool) -> Result<()> {
        let launch = if force || self.overrides.force_spawn {
            TTDaemonLaunch::Always
        } else {
            TTDaemonLaunch::IfNeeded
        };
        let socket_status = self.daemon.ensure_running(launch).await?;
        let client = self.connect_client(TTDaemonLaunch::Never).await?;
    ...
    pub async fn turn_get(&self, thread_id: &str, turn_id: &str) -> Result<()> {
        let client = self.ready_client().await?;
        let response = client
            .turn_attach(&tt_core::ipc::TurnAttachRequest {
                thread_id: thread_id.to_string(),
    ...
    pub async fn tt_worktree_prune(&self, selector: &str) -> Result<()> {
        let client = self.daemon_state_client().await?;
        ...
        match target {
            ResolvedTTWorktreeCleanupTarget::Workstream { workstream, .. } => {
                let response = client
                    .authority_workstream_delete(&ipc::AuthorityWorkstreamDeleteRequest {
                        command: authority::DeleteWorkstream {
                            metadata: Self::authority_command_metadata(),
                ...
                info!(
                    ...
                    "pruned tt worktree lane"
                );
                println!("surface: tt_worktree_prune");
            ...
            ResolvedTTWorktreeCleanupTarget::TrackedThread { tracked_thread, .. } => {
                let response = client
    ...
    pub async fn worktrees_list(&self) -> Result<()> {
        ...
        for workstream in response.workstreams {
            let worktree_path = workstream
                ...
                .as_ref()
                .and_then(|scope| {
                    Self::derive_worktree_path_from_tt_home(Path::new(&scope.tt_home))
                })
                .map(|path| path.display().to_string())
                .unwrap_or_else(|| "-".to_string());
    ...
    pub async fn planning_session_create(
        ...
        objective: String,
        research_status: tt_core::PlanningSessionResearchStatus,
    ...
    pub async fn planning_session_update_summary(
        ...
        let response = client
            .planning_session_update_summary(&ipc::PlanningSessionUpdateSummaryRequest {
                session_id: session_id.to_string(),
                summary: tt_core::PlanningSessionStructuredSummary {
                    objective,
                    requirements,
                    constraints,
                    non_goals,
    ...
    pub async fn tt_decision_list(
        ...
        let client = self.ready_client().await?;
        let decisions = Self::tt_decision_list_with_backend(
            client.as_ref(),
            thread_id,
            assignment_id,
            workstream_id,
        ...
        if decisions.is_empty() {
            if actionable_only {
                println!("no actionable tt supervisor decisions");
            } else {
                println!("no tt supervisor decisions");
            }
            return Ok(());
        }
    ...
    pub async fn tt_decision_history(
        &self,
        thread_id: Option<&str>,
        assignment_id: Option<&str>,
        include_superseded: bool,
        ...
        if decisions.is_empty() {
            println!("no tt supervisor decision history");
            return Ok(());
        }
    ...
    pub async fn tt_decision_get(&self, decision_id: &str) -> Result<()> {
        let client = self.ready_client().await?;
        let decision = Self::tt_decision_get_with_backend(client.as_ref(), decision_id).await?;
        let related = Self::tt_decision_list_with_backend(
            client.as_ref(),
            None,
    ...
    pub async fn tt_decision_propose_steer(
        &self,
        thread_id: &str,
        proposed_text: &str,
        requested_by: Option<String>,
        ...
        let result = Self::tt_decision_propose_steer_with_backend(
            client.as_ref(),
            thread_id,
            proposed_text,
            requested_by,
    ...
    pub async fn tt_decision_replace_pending_steer(
        &self,
        decision_id: &str,
        proposed_text: &str,
        requested_by: Option<String>,
        ...
        let result = Self::tt_decision_replace_pending_steer_with_backend(
            client.as_ref(),
            decision_id,
            proposed_text,
            requested_by,
    ...
    pub async fn tt_decision_record_no_action(
        &self,
        decision_id: &str,
        reviewed_by: Option<String>,
        review_note: Option<String>,
        ...
        let result = Self::tt_decision_record_no_action_with_backend(
            client.as_ref(),
            decision_id,
            reviewed_by,
            review_note,
    ...
    pub async fn tt_decision_manual_refresh(
        &self,
        thread_id: Option<&str>,
        assignment_id: Option<&str>,
        requested_by: Option<String>,
        ...
        let result = Self::tt_decision_manual_refresh_with_backend(
            client.as_ref(),
            thread_id,
            assignment_id,
            requested_by,
    ...
    pub async fn tt_decision_approve_and_send(
        &self,
        decision_id: &str,
        reviewed_by: Option<String>,
        review_note: Option<String>,
        ...
        let result = Self::tt_decision_approve_and_send_with_backend(
            client.as_ref(),
            decision_id,
            reviewed_by,
            review_note,
    ...
    pub async fn tt_decision_reject(
        &self,
        decision_id: &str,
        reviewed_by: Option<String>,
        review_note: Option<String>,
        ...
        let result = Self::tt_decision_reject_with_backend(
            client.as_ref(),
            decision_id,
            reviewed_by,
            review_note,
    ...
    async fn run_streaming_turn(
        ...
        let backend =
            TTSupervisorStreamingBackend::new(self.paths.clone(), &self.config, &self.overrides);
    ...
    async fn tt_decision_propose_steer_with_backend<B: SupervisorTTBackend + Sync>(
        backend: &B,
        thread_id: &str,
        proposed_text: &str,
        requested_by: Option<String>,
        ...
        let assignment = Self::resolve_active_tt_assignment_for_thread(backend, thread_id).await?;
    ...
    async fn tt_decision_list_with_backend<B: SupervisorTTBackend + Sync>(
        backend: &B,
        thread_id: Option<&str>,
        assignment_id: Option<&str>,
        workstream_id: Option<&str>,
        ...
        let response = backend
            .supervisor_decision_list(&ipc::SupervisorDecisionListRequest {
                ...
                tt_thread_id: thread_id.map(ToOwned::to_owned),
    ...
    async fn tt_decision_get_with_backend<B: SupervisorTTBackend + Sync>(
        backend: &B,
        decision_id: &str,
    ) -> Result<SupervisorTurnDecision> {
        let response = backend
    ...
    async fn tt_decision_record_no_action_with_backend<B: SupervisorTTBackend + Sync>(
        backend: &B,
        decision_id: &str,
        reviewed_by: Option<String>,
        review_note: Option<String>,
    ...
    async fn tt_decision_replace_pending_steer_with_backend<B: SupervisorTTBackend + Sync>(
        backend: &B,
        decision_id: &str,
        proposed_text: &str,
        requested_by: Option<String>,
    ...
    async fn tt_decision_manual_refresh_with_backend<B: SupervisorTTBackend + Sync>(
        backend: &B,
        thread_id: Option<&str>,
        assignment_id: Option<&str>,
        requested_by: Option<String>,
        ...
        let assignment_id = match (thread_id, assignment_id) {
            (Some(thread_id), None) => {
                Self::resolve_active_tt_assignment_for_thread(backend, thread_id)
                    .await?
                    .assignment_id
            }
    ...
    async fn tt_decision_approve_and_send_with_backend<B: SupervisorTTBackend + Sync>(
        backend: &B,
        decision_id: &str,
        reviewed_by: Option<String>,
        review_note: Option<String>,
    ...
    async fn tt_decision_reject_with_backend<B: SupervisorTTBackend + Sync>(
        backend: &B,
        decision_id: &str,
        reviewed_by: Option<String>,
        review_note: Option<String>,
    ...
    async fn resolve_active_tt_assignment_for_thread<B: SupervisorTTBackend + Sync>(
        backend: &B,
        thread_id: &str,
    ) -> Result<ipc::TTThreadAssignmentSummary> {
        let response = backend
            .tt_assignment_list(&ipc::TTAssignmentListRequest {
                tt_thread_id: Some(thread_id.to_string()),
                workstream_id: None,
                work_unit_id: None,
        ...
        let mut assignments = response.assignments.into_iter().filter(|assignment| {
            assignment.tt_thread_id == thread_id
                && assignment.status == TTThreadAssignmentStatus::Active
                && assignment.active
        });
        let assignment = assignments
            .next()
            .ok_or_else(|| anyhow!("no active TT assignment for thread `{thread_id}`"))?;
        if assignments.next().is_some() {
            bail!("multiple active TT assignments found for thread `{thread_id}`");
        }
        Ok(assignment)
    }
    ...
    fn authority_command_metadata() -> authority::CommandMetadata {
        authority::CommandMetadata::new(
            authority::OriginNodeId::parse(TT_CLI_NODE_ID).expect("static cli origin node id"),
    ...
    fn format_supervisor_turn_decision(
        ...
        let mut lines = vec![
            ...
            format!("thread_id: {}", decision.tt_thread_id),
    ...
    async fn ready_client(&self) -> Result<Arc<TTIpcClient>> {
        let client = self.connect_client_once(TTDaemonLaunch::Never).await?;
        client
            .daemon_connect()
            .await
            .map_err(|error| Error::new(error).context("connect TT daemon to TT"))?;
        Ok(client)
    }

    async fn daemon_state_client(&self) -> Result<Arc<TTIpcClient>> {
        self.connect_client_once(TTDaemonLaunch::Never).await
    }

    async fn connect_client(&self, launch: TTDaemonLaunch) -> Result<Arc<TTIpcClient>> {
        let mut last_error: Option<Error> = None;
        let mut delay = Duration::from_millis(100);

        for _ in 0..5 {
            match self.daemon.ensure_running(launch).await {
                Ok(_) => match TTIpcClient::connect(&self.paths).await {
                    Ok(client) => return Ok(client),
                    Err(error) => {
                        last_error = Some(Error::new(error).context("connect to TT daemon"));
                    }
                },
                Err(error) => {
        ...
        Err(last_error.unwrap_or_else(|| Error::msg("connect to TT daemon")))
    }

    async fn connect_client_once(&self, launch: TTDaemonLaunch) -> Result<Arc<TTIpcClient>> {
        let status = self
            .daemon
            .ensure_running(launch)
            .await
            .map_err(Error::new)?;
        TTIpcClient::connect(&self.paths).await.map_err(|error| {
            let detail = if status.running {
                format!(
                    "ttd appears to be running, but the socket at {} is stale or unreachable",
                    status.socket_path.display()
                )
            } else if status.stale_socket || status.stale_metadata || status.socket_exists {
                format!(
                    "ttd is not reachable and the socket at {} appears stale or unreachable",
                    status.socket_path.display()
                )
            } else {
                format!(
                    "ttd is not running; start it with `tt daemon start` ({})",
                    status.socket_path.display()
                )
            };
    ...
    fn daemon_event_line_parts(
        ...
        match event {
            ...
            ipc::DaemonEvent::TTAssignmentLifecycle { action, assignment } => (
                "tt_assignment",
                Some(assignment.tt_thread_id.as_str()),
            ...
            ipc::DaemonEvent::SupervisorDecisionLifecycle { action, decision } => (
                "supervisor_decision",
                Some(decision.tt_thread_id.as_str()),
    ...
    fn print_discovered_app_server(server: &LocalTTAppServer) {
    ...
    async fn discover_local_tt_app_servers() -> Result<Vec<LocalTTAppServer>> {
        ...
        for line in stdout.lines() {
            if let Some(server) = Self::parse_ss_tt_listener(line).await? {
                servers.push(server);
            }
        }
    ...
    async fn parse_ss_tt_listener(line: &str) -> Result<Option<LocalTTAppServer>> {
        ...
        let owner_listen_url = environment.get(TT_APP_SERVER_LISTEN_URL_ENV).cloned();
        let owner_started_at = environment.get(TT_APP_SERVER_STARTED_AT_ENV).cloned();
    ...
    fn print_draft_assignment(prefix: &str, draft: &tt_core::DraftAssignment) {
        println!(
...
fn build_execution_scope(
    ...
    Ok(
        match (
            ...
            _ => bail!("`--tt-home` is required when setting workstream execution scope"),
        },
    )
}
...
fn build_execution_scope_patch(
    ...
    if scope.tt_home.is_empty() {
        bail!("`--tt-home` is required when creating or updating workstream execution scope");
    }
    Ok(Some(Some(scope)))
}
...
fn print_workstream_execution_scope(scope: Option<&authority::WorkstreamExecutionScope>) {
    ...
    println!("execution_scope_tt_home: {}", scope.tt_home);
    println!(
...
fn print_workstream_runtime(runtime: &ipc::WorkstreamRuntimeSummary) {
    ...
    println!("runtime_tt_home: {}", runtime.tt_home);
    println!(
        "runtime_sqlite_home: {}",
...
mod tests {
    ...
    use chrono::Utc;
    use tt_core::{
        SupervisorPromptRenderArtifact, SupervisorPromptRenderSpec, SupervisorProposalFailure,
        SupervisorProposalFailureStage, SupervisorProposalStatus, SupervisorReasonerUsage,
        SupervisorResponseArtifact, SupervisorResponseContentPart, SupervisorResponseOutputItem,
        SupervisorTurnProposalKind, TTThreadBootstrapState, TTThreadSendPolicy,
    };

    #[derive(Debug, Default)]
    struct FakeSupervisorTTBackend {
        state: Mutex<FakeSupervisorTTState>,
        propose_error: Mutex<Option<String>>,
        approve_error: Mutex<Option<String>>,
    }

    #[derive(Debug, Default)]
    struct FakeSupervisorTTState {
        next_decision_id: usize,
        assignments: Vec<ipc::TTThreadAssignmentSummary>,
        decisions: BTreeMap<String, SupervisorTurnDecision>,
        last_approved_text: Option<String>,
    }

    #[async_trait]
    impl SupervisorTTBackend for FakeSupervisorTTBackend {
        async fn tt_assignment_list(
            &self,
            params: &ipc::TTAssignmentListRequest,
        ) -> Result<ipc::TTAssignmentListResponse> {
            ...
            let assignments = state
                ...
                .filter(|assignment| {
                    params
                        .tt_thread_id
                        .as_ref()
                        .is_none_or(|thread_id| &assignment.tt_thread_id == thread_id)
                        && (params.include_inactive || assignment.active)
                })
                .cloned()
                .collect();
            Ok(ipc::TTAssignmentListResponse { assignments })
        }
                ...
                .filter(|decision| {
                    ...
                    params
                        ...
                        && params
                            .tt_thread_id
                            .as_ref()
                            .is_none_or(|thread_id| &decision.tt_thread_id == thread_id)
        ...
        async fn supervisor_decision_manual_refresh(
            ...
            let assignment = state
                ...
                .cloned()
                .ok_or_else(|| anyhow!("unknown assignment `{}`", params.assignment_id))?;
            if assignment.status != TTThreadAssignmentStatus::Active {
                bail!(
                    "TT thread assignment `{}` is not active",
                    assignment.assignment_id
                );
            }
            ...
            let decision = SupervisorTurnDecision {
                ...
                tt_thread_id: assignment.tt_thread_id.clone(),
                ...
                proposed_text: Some(
                    "Continue under TT supervision for the assigned work unit.".to_string(),
                ),
    ...
    async fn cli_create_steer_with_authored_text() {
        let backend = FakeSupervisorTTBackend::default();
        seed_active_assignment(&backend, "assignment-1", "thread-1");

        let decision = SupervisorService::tt_decision_propose_steer_with_backend(
            &backend,
            "thread-1",
    ...
    fn unique_test_dir(label: &str) -> PathBuf {
        std::env::temp_dir().join(format!("tt-{label}-{}", uuid::Uuid::new_v4()))
    }
    ...
    async fn git_cleanup_prunes_worktree_and_deletes_branch() {
        ...
        fs::write(repo_root.join("README.md"), "tt\n").expect("seed file");
    ...
    fn effective_worktree_root_prefers_configured_default() {
        ...
        let configured_root = home.join("lanes/tt");
        let service = SupervisorService {
            paths,
            config: AppConfig {
                defaults: tt_core::config::DefaultsConfig {
                    worktree_root: Some(configured_root.clone()),
                    ..Default::default()
                },
                ..Default::default()
            },
            daemon: TTDaemonProcessManager::new(
                AppPaths::from_home(home.clone()),
                TTRuntimeOverrides::default(),
            ),
            overrides: TTRuntimeOverrides::default(),
        };
    ...
    async fn cli_create_steer_rejects_empty_text() {
        ...
        let error = SupervisorService::tt_decision_propose_steer_with_backend(
            &backend,
            "thread-1",
            "   \n\t  ",
            None,
    ...
    async fn cli_create_steer_rejects_unassigned_thread() {
        ...
        assert!(
            error
                .to_string()
                .contains("no active TT assignment for thread `thread-404`")
        );
    }
    ...
    async fn cli_replace_pending_steer_with_authored_text_supersedes_old_revision() {
        let backend = FakeSupervisorTTBackend::default();
        seed_active_assignment(&backend, "assignment-1", "thread-1");
        let original = SupervisorService::tt_decision_propose_steer_with_backend(
            &backend,
            "thread-1",
            "draft steer text",
            None,
        ...
        let replacement = SupervisorService::tt_decision_replace_pending_steer_with_backend(
            &backend,
    ...
    async fn cli_replace_pending_steer_rejects_empty_text() {
        ...
        let error = SupervisorService::tt_decision_replace_pending_steer_with_backend(
            &backend,
            &original.decision_id,
            " \n ",
            None,
    ...
    async fn filtered_decision_listing_by_status_kind_thread_and_assignment() {
        ...
        seed_assignment_with_context(
            ...
            "wu-1",
            "supervisor-a",
            TTThreadAssignmentStatus::Active,
        );
        seed_assignment_with_context(
            &backend,
            "assignment-2",
        ...
        seed_decision(
            ...
            SupervisorTurnDecision {
                ...
                tt_thread_id: "thread-2".to_string(),
        ...
        let by_status = SupervisorService::tt_decision_list_with_backend(
            &backend,
            None,
            None,
            None,
        ...
        let by_kind = SupervisorService::tt_decision_list_with_backend(
            &backend,
            None,
            None,
            None,
        ...
        let by_thread = SupervisorService::tt_decision_list_with_backend(
            &backend,
            Some("thread-1"),
            None,
            None,
        ...
        assert!(
            by_thread
                .iter()
                .all(|decision| decision.tt_thread_id == "thread-1")
        );

        let by_assignment = SupervisorService::tt_decision_list_with_backend(
            &backend,
            None,
            Some("assignment-2"),
            None,
    ...
    async fn actionable_queue_excludes_non_pending_decisions_and_supports_workflow_filters() {
        ...
        let queue = SupervisorService::tt_decision_list_with_backend(
            &backend,
            None,
            None,
            Some("ws-1"),
    ...
    async fn history_view_includes_superseded_steer_revisions_and_no_action_entries() {
        ...
        let history = SupervisorService::tt_decision_list_with_backend(
            &backend,
            Some("thread-1"),
            None,
            None,
    ...
    async fn queue_and_get_formatting_include_operator_useful_fields() {
        ...
        let listed = SupervisorService::tt_decision_list_with_backend(
            &backend,
            None,
            None,
            None,
    ...
    async fn approve_and_reject_continue_to_work_with_decisions_surfaced_from_queue() {
        ...
        .await
        .expect("queue");
        let approved = SupervisorService::tt_decision_approve_and_send_with_backend(
            &backend,
            &queue[0].decision_id,
            None,
            None,
        ...
        let rejected = SupervisorService::tt_decision_reject_with_backend(
            &backend,
            &queue[0].decision_id,
            None,
            None,
    ...
    async fn cli_record_no_action_from_pending_next_turn() {
        ...
        let decision = SupervisorService::tt_decision_record_no_action_with_backend(
            &backend,
            "std-next",
            None,
    ...
    async fn cli_manual_refresh_creates_pending_next_turn_from_recorded_no_action() {
        ...
        let decision = SupervisorService::tt_decision_manual_refresh_with_backend(
            &backend,
            Some("thread-1"),
            None,
            None,
    ...
    async fn cli_manual_refresh_rejects_missing_target_selection() {
        ...
        let error = SupervisorService::tt_decision_manual_refresh_with_backend(
    ...
    fn seed_active_assignment(
        backend: &FakeSupervisorTTBackend,
        assignment_id: &str,
        thread_id: &str,
    ) {
        seed_assignment_with_context(
    ...
    fn seed_assignment_with_context(
        ...
        work_unit_id: &str,
        supervisor_id: &str,
        status: TTThreadAssignmentStatus,
    ) {
        backend
            .state
            .lock()
            .expect("state")
            .assignments
            .push(ipc::TTThreadAssignmentSummary {
                assignment_id: assignment_id.to_string(),
                tt_thread_id: thread_id.to_string(),
                workstream_id: workstream_id.to_string(),
                work_unit_id: work_unit_id.to_string(),
                ...
                updated_at: Utc::now(),
                status,
                send_policy: TTThreadSendPolicy::HumanApprovalRequired,
                bootstrap_state: TTThreadBootstrapState::Sent,
                latest_basis_turn_id: Some("turn-active-1".to_string()),
                latest_decision_id: None,
                notes: None,
                active: status == TTThreadAssignmentStatus::Active,
            });
    }

    fn seed_decision(backend: &FakeSupervisorTTBackend, decision: SupervisorTurnDecision) {
        backend
            .state
            .lock()
            .expect("state")
    ...
    fn sample_decision(
        ...
        SupervisorTurnDecision {
            decision_id: decision_id.to_string(),
            assignment_id: assignment_id.to_string(),
            tt_thread_id: tt_thread_id.to_string(),
    ...
    fn assignment_summary_for_decision<'a>(
        assignments: &'a [ipc::TTThreadAssignmentSummary],
        decision: &SupervisorTurnDecision,
    ) -> Option<&'a ipc::TTThreadAssignmentSummary> {
        assignments
            .iter()
            .find(|assignment| assignment.assignment_id == decision.assignment_id)
    }
    ...
    fn sample_decision_summary(
        decision: &SupervisorTurnDecision,
        assignment: Option<&ipc::TTThreadAssignmentSummary>,
    ) -> ipc::SupervisorTurnDecisionSummary {
        ipc::SupervisorTurnDecisionSummary {
            decision_id: decision.decision_id.clone(),
            assignment_id: decision.assignment_id.clone(),
            tt_thread_id: decision.tt_thread_id.clone(),
        ...
        ipc::SupervisorProposalArtifactExport {
            ...
            artifact_detail: ipc::SupervisorProposalArtifactDetail {
                ...
                prompt_render: Some(SupervisorPromptRenderArtifact {
                    ...
                    instructions_text: "You are the TT supervisor reasoner.".to_string(),
