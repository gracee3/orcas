...
impl TTSkillBackend {
    ...
    fn service_overrides(&self, context: &SkillContext) -> RuntimeOverrides {
        ...
        overrides.tt_bin = context.tt_bin.clone();
...
#[async_trait(?Send)]
impl SkillBackend for TTSkillBackend {
    ...
    async fn tt_status(&self, context: &SkillContext, _: &TTStatusArgs) -> Result<SkillOutcome> {
        let service = self.service(context).await?;
        ...
        if !succeeded {
            bail!("tt status could not reach the session or app-server runtime");
        }
        Ok(Self::outcome("tt.status"))
    }

    async fn tt_spawn(
        &self,
        context: &SkillContext,
        args: &tt_skills::TTSpawnArgs,
    ) -> Result<SkillOutcome> {
        let service = self.service(context).await?;
        service
            .tt_spawn(
    ...
    async fn tt_app_server_status(
        ...
        Ok(Self::outcome("tt.app_server.status"))
    }
    ...
    async fn tt_app_server_start(
        ...
        Ok(Self::outcome("tt.app_server.start"))
    }
    ...
    async fn tt_app_server_stop(
        ...
        Ok(Self::outcome("tt.app_server.stop"))
    }
    ...
    async fn tt_app_server_restart(
        ...
        Ok(Self::outcome("tt.app_server.restart"))
    }
