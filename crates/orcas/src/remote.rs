...
fn server_client(global: &super::GlobalOptions) -> Result<OrcasServerClient> {
    let base_url = global
        .server_url
        .as_deref()
        .context("`--server-url` or `ORCAS_SERVER_URL` is required for remote commands")?;
    Ok(match global.operator_api_token.as_deref() {
        Some(token) => OrcasServerClient::with_operator_api_token(base_url, token),
        None => OrcasServerClient::new(base_url),
    })
}
...
fn delivery_status(kind: RemoteDeliveryStatusArg) -> orcas_core::ipc::NotificationDeliveryJobStatus {
    match kind {
        ...
        RemoteDeliveryStatusArg::Dispatched => {
            orcas_core::ipc::NotificationDeliveryJobStatus::Dispatched
        }
        RemoteDeliveryStatusArg::Delivered => {
            orcas_core::ipc::NotificationDeliveryJobStatus::Delivered
        }
        RemoteDeliveryStatusArg::Failed => orcas_core::ipc::NotificationDeliveryJobStatus::Failed,
        ...
        RemoteCommand::Actions { command } => match command {
            RemoteActionCommand::Submit(args) => {
                let idempotency_key = args
                    .idempotency_key
                    .unwrap_or_else(|| format!("orcas-remote-{}", Uuid::now_v7()));
                let response = client
