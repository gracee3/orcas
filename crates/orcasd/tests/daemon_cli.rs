...
async fn direct_daemon_flags_shape_runtime_status() {
    ...
    assert_eq!(status.codex_binary_path, "/bin/true");
    assert_eq!(status.codex_endpoint, "ws://127.0.0.1:4510");
}
...
async fn direct_daemon_flags_override_environment_values() {
    let daemon = TestDaemon::spawn_with_args(
        ...
        vec![
            ("ORCAS_CODEX_BIN".to_string(), "/bin/false".to_string()),
            (
    ...
    assert_eq!(status.codex_binary_path, "/bin/true");
    assert_eq!(status.codex_endpoint, "ws://127.0.0.1:4511");
}
