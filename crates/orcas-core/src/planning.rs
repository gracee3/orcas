...
use crate::{OrcasError, OrcasResult};
...
impl PlanningState {
    ...
    pub fn bootstrap_workstream(
        ...
        actor: impl Into<String>,
        now: DateTime<Utc>,
    ) -> OrcasResult<Option<WorkstreamPlan>> {
        ...
        match op {
            ...
            PlanRevisionOp::UpdateGoal { goal_id, patch } => {
                let goal = updated
                    ...
                    .ok_or_else(|| OrcasError::Protocol(format!("unknown goal `{goal_id}`")))?;
            ...
            PlanRevisionOp::UpdateItem { item_id, patch } => {
                let item = updated
                    ...
                    .ok_or_else(|| {
                        OrcasError::Protocol(format!("unknown plan item `{item_id}`"))
                    })?;
                if let Some(title) = patch.title.as_ref() {
            ...
            PlanRevisionOp::MoveItemBefore {
                ...
                let insert_at = match before_item_id {
                    Some(before_item_id) => updated
                        ...
                        .ok_or_else(|| {
                            OrcasError::Protocol(format!("unknown plan item `{before_item_id}`"))
                        })?,
