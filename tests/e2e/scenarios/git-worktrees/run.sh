...
e2e_prepare_live_tt_environment "gwt" 6000
...
Open a tracked-thread worktree lane, keep the lifecycle bounded, and verify the resulting workspace state matches what TT reports.
EOF
...
Use the declared tracked-thread workspace path as the source of truth. Create a tiny C program and Makefile there, keep the project buildable, and leave lifecycle actions to later TT workspace operations.
EOF
...
e2e_tt workunit thread get --tracked-thread "$tracked_thread_id" >"$tracked_before_stdout"
...
e2e_tt supervisor work reports get --report "$bootstrap_report_id" >"$bootstrap_report_get_stdout"
...
e2e_tt supervisor work assignments get --assignment "$bootstrap_assignment_id" >"$bootstrap_assignment_get_stdout"
...
e2e_tt workunit thread get --tracked-thread "$tracked_thread_id" >"$bootstrap_tracked_stdout"
...
grep -Eq '(^PASS$|Test passed)' "$bootstrap_make_test_stdout"
...
timeout "${TIMEOUT_SECONDS}s" "$e2e_bin_dir/tt.sh" workunit workspace prepare-workspace \
...
e2e_tt supervisor work reports get --report "$prepare_report_id" >"$reports_dir/prepare-report-get.txt"
e2e_tt supervisor work assignments get --assignment "$prepare_assignment_id" >"$reports_dir/prepare-assignment-get.txt"
e2e_tt workunit thread get --tracked-thread "$tracked_thread_id" >"$reports_dir/tracked-thread-after-prepare.txt"
...
timeout "${TIMEOUT_SECONDS}s" "$e2e_bin_dir/tt.sh" workunit workspace refresh-workspace \
  --tracked-thread "$tracked_thread_id" \
...
e2e_tt supervisor work reports get --report "$refresh_report_id" >"$reports_dir/refresh-report-get.txt"
e2e_tt supervisor work assignments get --assignment "$refresh_assignment_id" >"$reports_dir/refresh-assignment-get.txt"
e2e_tt workunit thread get --tracked-thread "$tracked_thread_id" >"$reports_dir/tracked-thread-after-refresh.txt"
...
timeout "${TIMEOUT_SECONDS}s" "$e2e_bin_dir/tt.sh" workunit workspace merge-prep \
  --tracked-thread "$tracked_thread_id" \
...
e2e_tt workunit thread get --tracked-thread "$tracked_thread_id" >"$reports_dir/tracked-thread-after-merge-prep.txt"

authorize_merge_stdout="$reports_dir/authorize-merge.txt"
timeout "${TIMEOUT_SECONDS}s" "$e2e_bin_dir/tt.sh" workunit workspace authorize-merge \
  --tracked-thread "$tracked_thread_id" \
...
e2e_tt workunit thread get --tracked-thread "$tracked_thread_id" >"$reports_dir/tracked-thread-after-authorize.txt"
...
timeout "${TIMEOUT_SECONDS}s" "$e2e_bin_dir/tt.sh" workunit workspace execute-landing \
  --tracked-thread "$tracked_thread_id" \
...
e2e_tt workunit thread get --tracked-thread "$tracked_thread_id" >"$reports_dir/tracked-thread-after-landing.txt"
...
timeout "${TIMEOUT_SECONDS}s" "$e2e_bin_dir/tt.sh" workunit workspace prune-workspace \
  --tracked-thread "$tracked_thread_id" \
...
e2e_tt workunit thread get --tracked-thread "$tracked_thread_id" >"$reports_dir/tracked-thread-after-prune.txt"
e2e_tt workunit get --workunit "$workunit_id" >"$reports_dir/workunit-before-complete.txt"
