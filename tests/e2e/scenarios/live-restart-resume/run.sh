...
e2e_prepare_live_tt_environment "lrs" 4700 16384

worktree_path="$E2E_SCENARIO_WORKTREES_DIR/lane"
repo_root="$E2E_SCENARIO_WORKTREES_DIR/lane-repo"
daemon_internal_log="$E2E_SCENARIO_TT_HOME/logs/ttd.log"
...
start_daemon() {
  local stdout_file="$1"
  e2e_tt daemon start --force-spawn >"$stdout_file" 2>&1 &
  daemon_pid=$!
}
...
for _ in $(seq 1 120); do
  active_turns_output="$("$e2e_bin_dir/tt.sh" turns list-active 2>/dev/null || true)"
...
e2e_tt tt threads read --thread "$thread_id" >"$threads_read_stdout"
e2e_tt workunit thread get --tracked-thread "$tracked_thread_id" >"$tracked_after_start_stdout"
...
e2e_tt daemon stop >"$stop_stdout"
wait_for_daemon_exit "$daemon_pid"
cp "$daemon_internal_log" "$reports_dir/ttd-before-restart.log"

start_daemon "$reports_dir/daemon-start-phase2.txt"
sleep 5
cp "$daemon_internal_log" "$reports_dir/ttd-after-restart.log"
...
for _ in $(seq 1 120); do
  "$e2e_bin_dir/tt.sh" turns get --thread "$thread_id" --turn "$turn_id" \
    >"$turn_get_after_stdout" 2>&1 || true
  reports_output_final="$("$e2e_bin_dir/tt.sh" supervisor work reports list-for-workunit --workunit "$workunit_id" 2>/dev/null || true)"
...
e2e_tt supervisor work assignments get --assignment "$assignment_id" >"$assignment_after_get_stdout"
...
e2e_tt workunit thread get --tracked-thread "$tracked_thread_id" >"$tracked_after_restart_stdout"
...
e2e_tt tt turns list-active >"$turns_active_after_stdout"
