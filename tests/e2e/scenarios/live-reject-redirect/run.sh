...
e2e_prepare_live_tt_environment "lrr" 5600 16384
...
workstream_output="$(
  e2e_tt workstreams create \
    ...
    --objective "Prove the governed live worker-to-supervisor loop on one bounded tracked-thread lane" \
    --priority normal
)"
...
tracked_output="$(
  e2e_add_tracked_thread_workspace \
    "$workunit_id" \
    "Live reject redirect lane" \
    "$repo_root" \
    "Dedicated tracked-thread worktree lane for the reject redirect scenario" \
    "$repo_root" \
    "$worktree_path" \
    "$branch_name" \
    "$base_ref" \
...
e2e_tt supervisor work proposals get --proposal "$proposal_id" >"$proposal_get_stdout"
...
model_draft_assignment_objective="$(e2e_field_value model_draft_assignment_objective "$proposal_get_stdout")"
...
e2e_tt supervisor work proposals artifact-summary --proposal "$proposal_id" >"$proposal_summary_stdout"
...
e2e_tt supervisor work proposals get --proposal "$proposal_id" >"$proposal_get_stdout"
approved_draft_assignment_objective="$(e2e_field_value approved_draft_assignment_objective "$proposal_get_stdout")"
...
e2e_tt supervisor work assignments get --assignment "$next_assignment_id" >"$next_assignment_get_stdout"
