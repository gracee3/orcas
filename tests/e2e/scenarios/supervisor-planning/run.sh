...
workstream_output="$(e2e_tt workstreams create --title "E2E Planning" --objective "Validate supervisor planning flow" --priority normal)"
workstream_id="$(printf '%s\n' "$workstream_output" | awk -F': ' '/^workstream_id:/ {print $2; exit}')"

workunit_output="$(e2e_tt workunit create --workstream "$workstream_id" --title "Planning work unit" --task "Draft a short implementation plan before execution")"
...
e2e_tt supervisor plan request-supervisor-context \
