...
Verify that TT can use a real tracked-thread worktree lane, land a bounded code change there, and carry the workspace through prepare, merge-prep, landing, and cleanup transitions with persisted state that matches the filesystem.

## What Is Seeded

1. A tiny git worktree is materialized under the scenario worktree output root from the TT repository itself.
2. The worktree contains a tiny C program, a shell test, and a Makefile with one obvious bug.
3. TT daemon state is started and one work unit is created in the workstream.
...
- The final persisted tracked-thread state, landing records, prune records, and decision records are produced by TT.
...
- TT can operate inside a tracked-thread worktree lane that is explicitly declared in authority state.
- TT can bind that lane automatically on the first live assignment and reflect it immediately through the workstream runtime view.
- TT can land a bounded code change into that lane and keep the local git state inspectable.
- TT can advance the lane through prepare-workspace to merge-prep to landing authorization to landing execution to prune cleanup.
- TT can reopen the lane between steps through persisted decision state without losing the tracked-thread binding.
- TT can prune the workspace and leave persisted state that honestly reflects the cleanup outcome.
