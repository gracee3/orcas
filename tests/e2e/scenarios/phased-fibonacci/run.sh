...
e2e_prepare_live_tt_environment "pfib" 6100
...
workstream_output="$(
  e2e_tt workstreams create \
    ...
    --objective "Validate a multi-phase tracked-thread workflow on one real live TT lane" \
    --priority high
)"
...
e2e_tt supervisor work reports get --report "$phase1_report_id" >"$phase1_report_get_stdout"
...
e2e_tt supervisor work assignments get --assignment "$phase1_assignment_id" >"$phase1_assignment_get_stdout"
...
git -C "$worktree_path" status --short | grep -v '^?? \.tt$' >"$phase1_git_status_stdout" || true
e2e_tt workunit thread get --tracked-thread "$tracked_thread_id" >"$phase1_tracked_stdout"
...
e2e_tt supervisor work reports get --report "$phase2_report_id" >"$phase2_report_get_stdout"
...
e2e_tt supervisor work assignments get --assignment "$phase2_assignment_id" >"$phase2_assignment_get_stdout"
...
e2e_tt workunit thread get --tracked-thread "$tracked_thread_id" >"$phase2_tracked_stdout"
...
e2e_tt supervisor work reports get --report "$phase3_report_id" >"$phase3_report_get_stdout"
...
e2e_tt supervisor work assignments get --assignment "$phase3_assignment_id" >"$phase3_assignment_get_stdout"
...
e2e_tt workunit thread get --tracked-thread "$tracked_thread_id" >"$phase3_tracked_stdout"
...
e2e_tt workunit get --workunit "$workunit_id" >"$reports_dir/workunit-after-completion.txt"
