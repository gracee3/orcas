...
e2e_prepare_live_tt_environment "lmpl" 5900 16384
...
e2e_tt workunit thread get --tracked-thread "$tracked_thread_id" >"$phase1_tracked_thread_after_stdout"
...
phase1_proposal_create_output="$(
  e2e_tt supervisor work proposals create \
    --workunit "$workunit_id" \
...
e2e_tt supervisor work proposals get --proposal "$phase1_proposal_id" >"$phase1_proposal_get_stdout"
...
phase1_proposal_approve_output="$(
  e2e_tt supervisor work proposals approve \
...
for _ in $(seq 1 120); do
  e2e_tt supervisor work assignments get --assignment "$phase1_next_assignment_id" >"$reports_dir/phase2-assignment-get.txt" 2>/dev/null || true
...
e2e_tt workunit thread get --tracked-thread "$tracked_thread_id" >"$phase2_tracked_thread_after_stdout"
